...
import { Instagram, MessageCircle, Phone, MapPin, Heart, Leaf, Cherry, Apple, Grape, Gift, Star, Truck, Clock, CheckCircle, ShoppingCart, Plus, Minus, X, Sparkles } from 'lucide-react';
...
function App() {
  ...
  const getProductBadges = (productId: string) => {
    ...
    if (sales >= 50) {
      badges.push({ text: '🏆 Bestseller', color: 'from-yellow-400 to-orange-400', textColor: 'text-white' });
    } else if (sales >= 30) {
      badges.push({ text: '⭐ Favorito', color: 'from-purple-400 to-pink-400', textColor: 'text-white' });
    } else if (sales >= 15) {
      badges.push({ text: '🔥 Popular', color: 'from-red-400 to-pink-400', textColor: 'text-white' });
    } else if (sales >= 10) {
      badges.push({ text: '✨ Más Vendido', color: 'from-blue-400 to-purple-400', textColor: 'text-white' });
    }
  ...
  const sizes = [
    ...
    { size: '220ml', price: 13000 }
  ];

  const addToCart = (product: any, size: string, price: number, type: 'mermelada' | 'aderezo') => {
  ...
  const generateWhatsAppMessage = () => {
    ...
    let message = `¡Hola! Soy ${customerInfo.name} y me gustaría hacer el siguiente pedido de Del Monte:\n\n`;
    ...
    if (mermeladasInCart.length > 0) {
      ...
      mermeladasInCart.forEach(item => {
        message += `• ${item.name} (${item.size}) - Cantidad: ${item.quantity} - $${(item.price * item.quantity).toLocaleString()}\n`;
      });
      message += '\n';
    }
    ...
    message += '¿Podrías confirmarme la disponibilidad y el costo de envío? ¡Gracias!';
  ...
  const handleWhatsAppContact = () => {
    window.open('https://wa.me/573105960000', '_blank');
  };
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {mermeladas.map((mermelada, index) => (
              <div key={mermelada.id} className="group animate-on-scroll" style={{animationDelay: `${index * 100}ms`}}>
                <div className="bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden transform hover:-translate-y-2 border border-gray-100">
                  <div className="relative">
                    ...
                    <div className="absolute top-4 left-4 space-y-2">
                      {getProductBadges(mermelada.id).map((badge, badgeIndex) => (
                        <div 
                          key={badgeIndex}
                  ...
                  <div className="p-6">
                    <h4 className="text-xl font-bold text-gray-800 mb-4 text-center">{mermelada.name}</h4>
                    ...
                    <div className="space-y-3">
                      {sizes.map((sizeOption) => (
                        <div key={sizeOption.size} className="flex items-center justify-between bg-gray-50 rounded-xl p-3 hover:bg-gray-100 transition-colors duration-200">
                          <div>
                            <span className="font-semibold text-gray-700">{sizeOption.size}</span>
                            <span className="text-2xl font-bold text-green-600 ml-3">${sizeOption.price.toLocaleString()}</span>
                          </div>
                          <button
                            onClick={() => addToCart(mermelada, sizeOption.size, sizeOption.price, 'mermelada')}
                            className={`bg-gradient-to-r from-orange-500 to-pink-500 hover:from-orange-600 hover:to-pink-600 text-white px-4 py-2 rounded-full flex items-center space-x-2 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 ${
                              showAddedAnimation === `${mermelada.id}-${sizeOption.size}` ? 'animate-pulse bg-green-500' : ''
                            }`}
                          >
            ...
            {aderezos.map((aderezo) => (
              <div key={aderezo.id} className="group animate-on-scroll">
                <div className="bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden transform hover:-translate-y-2 border border-gray-100">
                  ...
                  <div className="p-6">
                    <h4 className="text-xl font-bold text-gray-800 mb-4 text-center">{aderezo.name}</h4>
                    {/* Mostrar número de ventas si hay */}
                    ...
                    <div className="space-y-3">
                      {sizes.map((sizeOption) => (
                        <div key={sizeOption.size} className="flex items-center justify-between bg-gray-50 rounded-xl p-3 hover:bg-gray-100 transition-colors duration-200">
                          ...
                          <button
                            onClick={() => addToCart(aderezo, sizeOption.size, sizeOption.price, 'aderezo')}
                            className={`bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white px-4 py-2 rounded-full flex items-center space-x-2 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 ${
                              showAddedAnimation === `${aderezo.id}-${sizeOption.size}` ? 'animate-pulse bg-green-500' : ''
                            }`}
                          >
                            <Plus className="w-4 h-4" />
                            <span className="text-sm font-medium">Agregar</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-white rounded-2xl shadow-xl p-8 transform hover:-translate-y-2 transition-all duration-300 animate-on-scroll">
              ...
              <div className="space-y-4">
                <div className="flex items-center bg-blue-50 p-4 rounded-xl">
                  ...
                  <span className="text-gray-700 font-medium">Envíos a todo el Valle de Aburrá</span>
                </div>
                <div className="flex items-center bg-green-50 p-4 rounded-xl">
                  <CheckCircle className="w-6 h-6 text-green-500 mr-3" />
                  <span className="text-gray-700 font-medium">Cobertura nacional</span>
                </div>
              </div>
            </div>
            <div className="bg-gradient-to-br from-orange-100 via-pink-100 to-purple-100 rounded-2xl p-8 shadow-xl transform hover:-translate-y-2 transition-all duration-300 animate-on-scroll">
              <div className="flex items-center mb-6">
                ...
                <h4 className="text-2xl font-bold text-gray-800">Personalización</h4>
              </div>
              <p className="text-gray-700 text-lg font-medium leading-relaxed">
                ¿Tienes un sabor en mente? 
                <br />
                <span className="text-pink-600 font-bold">¡Lo preparamos a tu gusto!</span>
              </p>
        ...
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            <div className="bg-white rounded-2xl shadow-xl p-8 transform hover:-translate-y-2 transition-all duration-300 animate-on-scroll">
              ...
              <h4 className="text-xl font-bold mb-3 text-gray-800">Instagram</h4>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 transform hover:-translate-y-2 transition-all duration-300 animate-on-scroll">
              ...
              <p className="text-gray-600 text-lg">310 596 0000</p>
            </div>
          </div>
        </div>
      </section>
            ...
            <div className="p-6 max-h-96 overflow-y-auto">
              {cart.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-400">¡Agrega algunas mermeladas deliciosas!</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {cart.map((item) => (
                    <div key={item.id} className="bg-gray-50 rounded-xl p-4 flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600">{item.size} - ${item.price.toLocaleString()}</p>
            ...
            <div className="p-6">
              ...
              <div className="mt-6 bg-gradient-to-r from-orange-100 to-pink-100 rounded-xl p-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-bold text-gray-800">Total del pedido:</span>
